...
const double _kMenuItemHeight = 30;
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    final double width = math.min(widthDevice * .8, widthDevice * .8);
        ...
        builder: (BuildContext context) {
          return Padding(
            padding: EdgeInsets.only(
              top: selectedIndex == 0 ? 1 * buttonRect.height : 0.0,
              right: textDirection == TextDirection.rtl ? width * .1 : 0,
              left: textDirection == TextDirection.ltr ? width * .1 : 0,
            ),
    ...
    assert(widget.value == null ||
        widget.items
                .where((DropdownMenuItem<T> item) =>
                    item.value.toString() == widget.value.toString())
                .length ==
            1);
    _selectedIndex = null;
